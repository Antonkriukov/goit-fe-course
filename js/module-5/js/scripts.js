...
const getUsersByGender = (arr, gender) =>
  arr.filter(y => y.gender === gender).map(y => y.name);

console.log(getUsersByGender(users, 'male')); // [ 'Moore Hensley', 'Ross Vazquez', 'Carey Barr', 'Blackburn Dotson' ]

// Получить массив только неактивных пользователей (поле isActive)

const getInactiveUsers = arr => arr.filter(a => !a.isActive);

console.log(getInactiveUsers(users)); // [объект Moore Hensley, объект Ross Vazquez, объект Blackburn Dotson]

//   Получить пользоваля (не массив) по email (поле email, он уникальный)

const getUserByEmail = (arr, email) => arr.find(z => z.email === email);
...
const getUsersWithAge = (arr, min, max) =>
  arr.filter(b => min > b.age && max < b.age);
...
const getUsersByFriend = (arr, key, name) =>
  arr
    .filter(
      user => (
        (user = user[key]),
        Array.isArray(user) ? user.includes(name) : user === name
      ),
    )
    .map(user => user.name);
console.log(getUsersByFriend(users, 'friends', 'Briana Decker')); // [ 'Sharlene Bush', 'Sheree Anthony' ]
console.log(getUsersByFriend(users, 'friends', 'Goldie Gentry')); // [ 'Elma Head', 'Sheree Anthony' ]
